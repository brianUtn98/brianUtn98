### Brian Gabriel Monroy - @brianUtn98

<!--
**brianUtn98/brianUtn98** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
-->
Blockchain Engineer - Backend Engineer
* +3 Years of experience in Web2.0 developer (Typescript). 💻
* +2 Years of experience in Web3.0 Blockchain Engineer (Typescript, EVM and Solana). ⛓️
* Kubernetes & Infra as code (EKS, Timoni) 🏗️
* AWS (S3, EC2, EKS, Cognito, between others) 🏛️
* Microservices
* Hexagonal Architecture
* Grafana & Prometheus Monitoring and Alerting 🚒
  
📫 Contact me - [Linkedin](https://www.linkedin.com/in/brian-gabriel-monroy-402571209/) - <a href="mailto:brian.gmonroy98@gmail.com">brian.gmonroy98@gmail.com</a>
